...
run_test "Create instance" POST "/instances" '{"name":"ct'$(date +%s | tail -c 4)'","source":{"type":"image","alias":"ubuntu/24.04"},"config":{},"type":"container"}' "created"
...
run_test "Create storage pool (stub)" POST "/storage-pools" '{"name":"test-api-pool-'$(date +%s)'-'$(openssl rand -hex 2)'","driver":"dir","config":{}}' "created"
...
run_test "Create network (stub)" POST "/networks" '{"name":"n'$(date +%s | tail -c 4)'","type":"bridge","config":{}}' "created"
...
run_test "Create profile (stub)" POST "/profiles" '{"name":"test-profile-'$(date +%s)'-'$(openssl rand -hex 2)'","config":{}}' "created"
